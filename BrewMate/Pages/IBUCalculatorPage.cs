...
namespace BrewMate
{
	public class IBUCalculatorPage : GreenGradientPage
	{
		...
		public IBUCalculatorPage ()
		{
			...
			this.Content = new ScrollView () { Content = pageContents };

			//Stepper Controls

			gravityStepper.ValueChanged += (object sender, ValueChangedEventArgs e) => {
			...
			boilGravityEntry.TextChanged += (object sender, TextChangedEventArgs e) => {
			...
			boilVolumeEntry.TextChanged += (object sender, TextChangedEventArgs e) => {
			...
			calculateIBUButton.Clicked += (object sender, EventArgs e) => {
