...
namespace BrewMate
{
	public class SRMColorCalculator
	{
		public void SRMCalculator (Label lowSRM, Label highSRM)
		{
			double srm = 0;
			int srmInteger;
		...
		public MashCalculatedModel CalculateSRM(TableView grainsInTable, Entry mashVolumeEntry)
		{
			...
			for (int i = 1; i<grainsInTable.Root [0].Count; i++) {
				...
				srm = children [1] as Label;
				grainWeight = children [3] as Entry;

				MCU = (Convert.ToDouble (srm.Text) * Convert.ToDouble (grainWeight.Text))/Convert.ToDouble(mashVolumeEntry.Text);
				totalMCU += MCU;
			}
			...
			return new MashCalculatedModel () {
				srmColor = finalColor,
				srmInt = srmInt
			};
		}
